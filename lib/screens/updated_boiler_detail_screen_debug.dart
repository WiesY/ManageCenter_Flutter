...
Future<void> _selectDateTime(BuildContext context) async {
  ...
  final DateTime? pickedDate = await showDatePicker(
    ...
    locale: const Locale('ru', 'RU'), // Добавляем русскую локализацию
      ...
      builder: (context, child) {
        return Theme(
          ...
          child: Localizations.override(
            ...
            child: MediaQuery(
              // Принудительно устанавливаем 24-часовой формат
                            ...
                            itemBuilder: (context, index) {
                              ...
                              return Card(
                                ...
                                child: CheckboxListTile(
                                  title: Text(
                                    parameter.paramDescription, // Показываем название параметра
                                    style: TextStyle(
                                      fontSize: 13,
  ...
  Widget _buildParameterValuesList(List<BoilerParameterValue> values) {
    // ОТЛАДОЧНАЯ ИНФОРМАЦИЯ - выводим в консоль что получили
    print('=== ОТЛАДКА ЗНАЧЕНИЙ ПАРАМЕТРОВ ===');
    print('Всего значений: ${values.length}');
    for (var i = 0; i < values.length && i < 5; i++) {
      final value = values[i];
      print('Значение $i:');
      print('  - parameterId: ${value.parameterId}');
      print('  - parameterDescription: "${value.parameterDescription}"');
      print('  - value: ${value.value}');
      print('  - valueType: ${value.valueType}');
      print('  - displayValue: ${value.displayValue}');
    }
    print('=====================================');
                              ...
                              return Container(
                                ...
                                child: _buildParameterRow(
                                  value.parameterDescription.isEmpty 
                                    ? 'Параметр ID: ${value.parameterId}' // Fallback если описание пустое
                                    : value.parameterDescription, // Показываем полное название параметра
                                  value.displayValue,
                                  value.valueType,
                                  value.parameterId, // Добавляем ID для отладки
                                ),
                              );
                            },
                          ),
      ...
      children: [
        Expanded(
          ...
          child: Column(
            ...
            children: [
              Text(
                label, // Полное название параметра или fallback
                style: const TextStyle(
                  fontWeight: FontWeight.w500,
                  fontSize: 13,
                ),
