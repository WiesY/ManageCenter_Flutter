...
class ApiService {
  static const String baseUrl = 'https://boiler.nwwork.site/api/v1';
  static const String apiV1 = '/api/v1';
  static const String apiV2 = 'https://boiler-v2.nwwork.site/api';

  Future<TokenResponse> login(String login, String password) async {
    try {
      // 1. Собираем данные об устройстве
      String? deviceName;
      String? deviceId;
      String? osVersion;
      ...
      try {
        if (Platform.isAndroid) {
          ...
          deviceName =
              '${androidInfo.manufacturer} ${androidInfo.model}'; // "Xiaomi M2101K7BNY"
          deviceId = androidInfo.id; // Уникальный ID сборки
          osVersion = 'Android ${androidInfo.version.release}';
        } else if (Platform.isIOS) {
          IosDeviceInfo iosInfo = await deviceInfo.iosInfo;
          deviceName = iosInfo.name; // "iPhone 13 Pro"
          deviceId = iosInfo.identifierForVendor; // Уникальный ID вендора
          osVersion = 'iOS ${iosInfo.systemVersion}';
        } else if (Platform.isWindows) {
        WindowsDeviceInfo windowsInfo = await deviceInfo.windowsInfo;
        deviceName = windowsInfo.computerName; // Имя компьютера (например, DESKTOP-HQ123)
        deviceId = windowsInfo.deviceId;       // Уникальный ID системы (MachineGuid)
        osVersion = '${windowsInfo.productName} (Build ${windowsInfo.buildNumber})'; // Например: Windows 10 Pro (Build 19044)
      }
      } catch (e) {
        print('Ошибка получения инфо об устройстве: $e');
        deviceName = 'Unknown Device';
      }

      final response = await http.post(
        Uri.parse('$apiV2/Auth/Login'),
...
// Исправленный метод getBoilerParameterValues
  Future<BoilerHistoryResponse> getBoilerParameterValues(
    ...
    try {
      ...
      final uri =
          Uri.parse('$baseUrl/BoilerParameters/$boilerId/Values').replace(
      ...
      switch (response.statusCode) {
        case 200:
          ...
          print(
              'Created BoilerHistoryResponse with ${historyResponse.historyNodeValues.length} values');
...
// Получение истории значений для конкретного параметра
  Future<List<BoilerParameterValue>> getParameterHistoryValues(
    ...
    try {
      ...
      final uri =
          Uri.parse('$baseUrl/BoilerParameters/$boilerId/$parameterId/Values')
              .replace(
        queryParameters: queryParams,
      );
      ...
      switch (response.statusCode) {
        case 200:
          ...
          if (data is Map<String, dynamic>) {
            ...
            if (data.containsKey('historyNodeValues') &&
                data['historyNodeValues'] is List) {
              final List<dynamic> values = data['historyNodeValues'];
              print(
                  'Received ${values.length} parameter values from historyNodeValues');
              return values
                  .map((item) => BoilerParameterValue.fromJson(item))
                  .toList();
...
// Получение истории значений параметров по типу объекта
  Future<List<BoilerTypeCompareValues>> getBoilerParametersByTypeCompareValues(
      ...
      {bool includeUngrouped = true}) async {
    try {
      print(
          'Getting parameter values for boiler type $boilerTypeId at $compareDateTime');
      ...
      switch (response.statusCode) {
        case 200:
          ...
          return data
              .map((boilerData) => BoilerTypeCompareValues.fromJson(boilerData))
              .toList();
        ...
        case 403:
          throw Exception(
              'Пользователь должен иметь доступ к указанной котельной');
        case 404:
...
// Получение списка инцидентов
  Future<List<IncidentModel>> getIncidents(
    ...
    try {
      ...
      final uri =
          Uri.parse('https://boiler-v2.nwwork.site/api/Incidents').replace(
        queryParameters: queryParams,
      );
      ...
      switch (response.statusCode) {
        case 200:
          ...
          final incidents =
              data.map((json) => IncidentModel.fromJson(json)).toList();
...
// Сброс инцидента (перевод из активного в архив)
  Future<void> resetIncident(String token, int incidentId) async {
    try {
      ...
      final response = await http.post(
        Uri.parse(
            'https://boiler-v2.nwwork.site/api/Incidents/$incidentId/reset'),
        headers: {
          'Authorization': 'Bearer $token',
          'Content-Type': 'application/json',
        },
      ...
      switch (response.statusCode) {
        ...
        case 400:
          ...
          final errorMessage =
              response.body.isNotEmpty ? response.body : 'Некорректный запрос';
...
// Получение группы по ID
  Future<ParameterGroup> getParameterGroupById(
      String token, int paramGroupId) async {
    try {
      final response = await http.get(
...
// Создание новой группы параметров
  Future<ParameterGroup> createParameterGroup(
      String token, String name, String color, String? iconFileName) async {
    try {
      var request =
          http.MultipartRequest('POST', Uri.parse('$baseUrl/ParamGroups'));
...
// Обновление группы параметров
  Future<ParameterGroup> updateParameterGroup(String token, int paramGroupId,
      String name, String color, String? iconFileName) async {
    try {
      var request = http.MultipartRequest(
          'PUT', Uri.parse('$baseUrl/ParamGroups/$paramGroupId'));
...
// Получение иконки группы по имени файла
  Future<String> getParameterGroupIconByName(
      String token, String? fileName, bool isDownload) async {
    try {
      final queryParams = {
...
// Получение иконки группы по ID группы
  Future<String> getParameterGroupIconById(
      String token, int paramGroupId, bool isDownload) async {
    try {
      final queryParams = {
        'isDownload': isDownload.toString(),
      };
...
// Изменение группы у параметров
  Future<void> updateParametersGroup(
      String token, int groupId, List<int> parametersId) async {
    try {
      final response = await http.put(
...
//-------управление ролями--------
  ...
  Future<Role> createRole(String token, Map<String, dynamic> roleData) async {
    final response = await http.post(
      ...
      body: json.encode(
          roleData), // {name, canAccessAllBoilers: bool, canManageAccounts: bool, canManageBoilers: bool}
    );
    if (response.statusCode == 200) {
  ...
  Future<Role> updateRole(
      String token, int roleId, Map<String, dynamic> roleData) async {
    final response = await http.put(
  ...
  Future<List<BoilerType>> getAllBoilerTypes(String token) async {
    try {
      ...
      if (response.statusCode == 200) {
        final List<dynamic> data = json.decode(response.body);
        final boilerTypes =
            data.map((json) => BoilerType.fromJson(json)).toList();
        return boilerTypes;
      } else if (response.statusCode == 401) {
        throw Exception('Некорректный токен авторизации');
      } else {
...
// Редактирование типа объекта
  Future<void> updateBoilerType(
      String token, int boilerTypeId, String name) async {
    try {
      final response = await http.put(
        Uri.parse('$baseUrl/BoilerTypes/$boilerTypeId'),
        headers: {
...
// Создание нового объекта
  Future<void> createBoiler(
      String token, Map<String, dynamic> boilerData) async {
    try {
      final response = await http.post(
...
// Получение объекта по ID
  Future<BoilerListItem> getBoilerById(String token, int boilerId) async {
    try {
      print('Запрос актуального статуса котельной ID: $boilerId');

      final response = await http.get(
        Uri.parse('$apiV2/boilers/$boilerId/details'),
        headers: {
          'Authorization': 'Bearer $token',
          'Content-Type': 'application/json',
        },
      ...
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        // 3. Возвращаем модель с правильными флагами
      ...
      print('Ошибка при получении деталей объекта: $e');
      rethrow;
    }
  }
...
// Обновление объекта
  Future<void> updateBoiler(
      String token, int boilerId, Map<String, dynamic> boilerData) async {
    try {
      final response = await http.put(
